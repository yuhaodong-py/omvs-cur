...
def get_descriptors(smiles):
    ...
    if mol is not None:
        ...
        return [
            ...
            Chem.rdMolDescriptors.CalcLabuteASA(mol),  # 考虑氢原子的 Labute 极性表面积
...
# 使用 GridSearchCV
grid_search = GridSearchCV(estimator=DecisionTreeClassifier(random_state=42), param_grid=param_grid, cv=3, scoring='accuracy', verbose=2)
...
# 绘制 SHAP 汇总图
shap.summary_plot(shap_values[0], X_train, feature_names=[f'descriptor_{i}' for i in range(X_descriptors.shape[1])] + [f'bit_{i}' for i in range(X_fingerprints.shape[1])])

# 绘制决策图
shap.decision_plot(expected_value, shap_values[0][0], X_train[0], feature_names=[f'descriptor_{i}' for i in range(X_descriptors.shape[1])] + [f'bit_{i}' for i in range(X_fingerprints.shape[1])])
